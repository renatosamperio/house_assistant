...
from GetVoiceOrder import GetVoiceOrder

class ServiceGetVoiceOrder(ServiceHandler):
  ''' Service for Gets a void order'''
  ...
  def DeserializeAction(self, msg):
    ...
    try:
      isForDevice = msg['header']['service_name'] == 'voice_order' or msg['header']['service_name'] == 'all'
  ...
  def GetActionHandler(self, msg):
    ''' '''    
    self.logger.debug("Creating a Gets a void order")
    result	="failure"
    deviceAction= None
	...
	start_state = 'created'	
      ...
      # Creating service object and notify
      deviceAction = GetVoiceOrder(**args)
  ...
  def close(self):
    ...
    self.logger.debug("Stopping Gets a void order service")
